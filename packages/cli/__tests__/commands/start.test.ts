...
describe("start command — orchestrator strategy delegation", () => {
  it("always delegates session handling to spawnOrchestrator", async () => {
    mockConfigRef.current = makeConfig({
      "my-app": makeProject({ orchestratorSessionStrategy: "delete" }),
    });
    mockSessionManager.get.mockResolvedValue({
      id: "app-orchestrator",
      status: "working",
      runtimeHandle: { id: "rt-existing" },
    });
    mockSessionManager.spawnOrchestrator.mockResolvedValue({
      id: "app-orchestrator",
      runtimeHandle: { id: "rt-new" },
    });
