...
async function runStartup(
  ...
  const selectedAgent = project.agent ?? config.defaults.agent;
  ...
  if (opts?.orchestrator !== false) {
    if (selectedAgent === "opencode") {
      await preflight.checkOpenCodeInstalled();
      const subagent = project.agentConfig?.subagent;
      if (typeof subagent === "string" && subagent.length > 0) {
        await preflight.checkOpenCodeSubagent(subagent);
      }
    }
